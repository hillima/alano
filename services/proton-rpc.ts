...
class ProtonJs {
  ...
  async getAccountDataPic(chainAccount: string): Promise<any> {
    const { rows } = await this.rpc.get_table_rows({
      json: true,
      code: 'eosio.proton',
      scope: 'eosio.proton',
    ...
    return rows && rows.length && rows[0].acc === chainAccount
      ? {name: rows[0].name, avatar: rows[0].avatar, isVerified: rows[0].verified}
      : undefined;
  }
  ...
  getColMutations = async (colName: string): Promise<string> => {
    try {
      const res = await this.rpc.get_table_rows({
        json: true,
        code: 'dgcontract',
        scope: 'dgcontract',
        table: 'collections',
        lower_bound: encodeName(colName, false),
        upper_bound: encodeName(colName, false),
        limit: 1,
        reverse: false,
        show_payer: false,
      });

      const data = res.rows.length == 0 ? {collection_name: colName, like_cnt: 0, watch_cnt: 0} : res.rows[0];
  ...
  getUserMutations = async (userName: string): Promise<string> => {
    try {
      const res = await this.rpc.get_table_rows({
        json: true,
        code: 'dgcontract',
        scope: 'dgcontract',
        table: 'names',
        lower_bound: encodeName(userName, false),
        upper_bound: encodeName(userName, false),
        limit: 1,
        reverse: false,
        show_payer: false,
      });

      const data = res.rows.length == 0 ? {actor: userName, colwatches: []} : res.rows[0];
  ...
  getTransaction = async (id: string): Promise<any> => {
    try {
      const data = this.rpc.history_get_transaction(id);
      return data;
    } catch(e) {
      return e;
    }
