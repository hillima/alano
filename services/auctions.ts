...
export const getUserAuctions = async (name): Promise<Auction[]> => {
  try {
    const result = await axios.post(`/api/user/auctions`, {
      data: {
        name: name
      }
    });
...
export const getAuctions = async ({limit, page, state}): Promise<Auction[]> => {
  try {
    const result = await axios.post(`/api/auctions`, {
      data: {
        limit,
        page,
        state
...
export const getAuctionDetail = async (auctionId): Promise<Auction[]> => {
  try {
    const result = await getFromApi<Auction[]>(
      `${process.env.NEXT_PUBLIC_NFT_ENDPOINT}/atomicmarket/v1/auctions/${auctionId}`
    );
    return result.data;
  } catch (e) {
    throw new Error(e);
