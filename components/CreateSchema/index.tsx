...
import { DEFAULT_SCHEMA, SM_FILE_UPLOAD_TYPES_TEXT } from '../../utils/constants';
        ...
        {uploadedFile == null && updatedImage == null ? 
          <Column style={{paddingTop: '54px'}}>
            ...
            <>
              ...
              <img src={updatedImage !== null ?  `https://ipfs.io/ipfs/${updatedImage}` : uploadedFile !== null ? URL.createObjectURL(uploadedFile) : ''} style={{width: '100%', height: '100%', objectFit: 'contain', borderRadius: '16px'}}/>
            </>
          }
          </div>
      </div>
...
const CreateSchema: FC<{
  ...
  const { currentUser } = useAuthContext();
  const author = currentUser ? currentUser.actor : '';
  const [schemaName, setName] = useState<string>('');
  ...
  const save = async () =>{
    ...
    await ProtonSDK.createSchema({
      author,
      schema_name: schemaName.toLowerCase(),
      collection_name: selectedCollection.collection_name,
      schema_format: newArr
    }).then(res =>{
      if(res.success){
        window.location.reload();
        return setPageState(CREATE_PAGE_STATES.SELECT_COLLECTION);
      }
    });
  };
  return (
    <>
      ...
      <BackText onClick={() => setPageState(CREATE_PAGE_STATES.SELECT_COLLECTION)}><img src="/left.svg" />Back</BackText>
      <div style={{maxWidth: '644px'}}>
      <div style={{color: '#FFF', marginTop: '44px', marginBottom: '44px'}}>
        <p style={{fontSize: '28px', marginBottom: '15px'}}>Create a schema (Properties)</p>
        <p style={{fontSize: '14px', marginBottom: '15px'}}>Schemas define which kind of attributes (properties) your NFT’s will have on the blockchain.</p>
        <p style={{fontSize: '14px', marginBottom: '15px'}}>1. Choose any name, click <span style={{color: '#7B0A75'}}>Create Schema</span></p>
        <p style={{fontSize: '14px', marginBottom: '15px'}}>2. <span style={{color: '#7B0A75'}}>Create NFT</span>. You can add attributes later!</p>
      </div>
      <CollectionBox>
      ...
      <div style={{display: 'flex', alignItems: 'center'}}>
        <InputField
          ...
          setValue={setName}
        />
        <HalfButton style={{width:'201px', margin: '10px 0 12px 19px'}} onClick={save}>Create Schema</HalfButton>
      </div>
      <div>
