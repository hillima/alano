    ...
    case 'POST':
        {
            try {
                ...
                let newAssets = [];
                while(hasAssets){
                  let assets = await axios.get(`https://proton.api.atomicassets.io/atomicassets/v1/assets?owner=${user}&page=${page}&limit=100&order=desc&sort=name&collection_blacklist=${whiteList.join(',')}`);
